...
do_once.helve_lib = function ()
    ...
    local fire_charges_lines = {}
    fire_charges_lines[#fire_charges_lines + 1] = "[ITEM_AMMO:ITEM_AMMO_FIRE_CHARGE]"
    add_generated_info(fire_charges_lines)
    fire_charges_lines[#fire_charges_lines + 1] = "[NAME:fire charge:fire charges]"
    fire_charges_lines[#fire_charges_lines + 1] = "[CLASS:FIRE_CHARGE]"
    fire_charges_lines[#fire_charges_lines + 1] = "[SIZE:10]"
    fire_charges_lines[#fire_charges_lines + 1] = "[ATTACK:BLUNT:10:10:strike:strikes:NO_SUB:5000]"

    raws.register_items(fire_charges_lines)

    print_table(fire_charges_lines)

    -- Register items with DF API
    -- TODO: Why do I need to wrap this in a table?
    raws.register_items({
        weapon_lines
    })
    ...
    materials_lines[#materials_lines+1] = "[STATE_NAME:ALL_SOLID:black powder]"
    ...
    -- Output container
    reactions_lines[#reactions_lines + 1] = "[REAGENT:bag:1:BAG:NONE:NONE:NONE]"
    reactions_lines[#reactions_lines + 1] = "   [EMPTY]"
    ...
    reactions_lines[#reactions_lines + 1] = "   [PRODUCT_TO_CONTAINER:bag]"
    reactions_lines[#reactions_lines + 1] = "   [PRODUCT_DIMENSION:150]"

    reactions_lines[#reactions_lines + 1] = "[REACTION:MAKE_FIRE_CHARGE]"
    add_generated_info(reactions_lines)
    reactions_lines[#reactions_lines + 1] = "[FORTRESS_MODE_ENABLED]"
    reactions_lines[#reactions_lines + 1] = "[NAME:Make fire charge with black powder]"
    reactions_lines[#reactions_lines + 1] = "[BUILDING:BOWYER:NONE]"
    reactions_lines[#reactions_lines + 1] = "[SKILL:BOWYER]"

    reactions_lines[#reactions_lines + 1] = "[REAGENT:black powder:150:POWDER_MISC:NONE:INORGANIC:BLACK_POWDER]"
    reactions_lines[#reactions_lines + 1] = "[REAGENT:black powder container:1:NONE:NONE:NONE:NONE]"
    reactions_lines[#reactions_lines + 1] = "   [CONTAINS:black powder]"
    reactions_lines[#reactions_lines + 1] = "   [PRESERVE_REAGENT]"
    reactions_lines[#reactions_lines + 1] = "   [DOES_NOT_DETERMINE_PRODUCT_AMOUNT]"

    reactions_lines[#reactions_lines + 1] = "[PRODUCT:100:5:AMMO:ITEM_AMMO_FIRE_CHARGE:GET_MATERIAL_FROM_REAGENT:black powder]"
    reactions_lines[#reactions_lines + 1] = "     [PRODUCT_TOKEN:fire_charge]"
    reactions_lines[#reactions_lines + 1] = "     [PRODUCT_DIMENSION:150]"
    ...
    interactions_lines[#interactions_lines + 1] = "     [IS_CDI:WAIT_PERIOD:0]"
